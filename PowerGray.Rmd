...
top15 = function(area, type) {
  ...
  artists = rev(rownames(flipped.cy)[order(flipped.cy[,1], decreasing = TRUE)])
  appearances = rev(flipped.cy[order(flipped.cy[,1], decreasing = TRUE),])
...
---
```{r}
ed_sheeran_daily <- data %>%
        filter(Country == "USA", Artist == "Ed Sheeran", Position <= 100)
