...
+ Create a network of countries 
```{r, eval = FALSE}
countries = data$Country
country.graph <- graph_from_adjacency_matrix(as.dist(love_adjacency[,-1]))

ggraph(LA_graph, layout = "kk") +
  geom_edge_link() +
  geom_node_label(aes(label = actor_names), size = 2) +
...
+ Create a map base of the world
```{r, eval = FALSE}
center = 160
worldmap = map_data("world")

ggplot() + 
  geom_polygon(worldmap, aes(x = 29.4873047, y = 27.0591258)) +
  scale_y_continuous(limits = c(-85, 85))

map.base = get_map(location = c(29.4873047, 27.0591258),
                   source = "google")
